...
inline void SetupLUT::EvaluateTTGate(uint32_t gateid) {
	...
	if(m_bPlaySender) {
		...
		for(uint32_t i = 0; i < nvals; i++) {
...
#ifdef DEBUG_SPLUT
			std::cout << "Client parent input shares to gate " << gateid << ": " <<  std::hex << gate_shares << std::dec << std::endl;
#endif
			//std::cout << "Choice inputs: " << std::hex << choice_bits << std::dec << std::endl;
			...
			//std::cout << "Sending updated choice: " << std::hex << gate_shares << std::dec << std::endl;
...
#ifdef DEBUG_SPLUT
		std::cout << "Setting TT gate as receiver" << std::endl;
#endif
	...
	for(uint32_t i = 0; i < m_vTTGates.size(); i++) {
		...
		for(uint32_t g = 0; g < m_vTTGates[i].size(); g++) {
			...
			for(uint32_t j = 0; j < ceil_divide(gate->nvals, typebitlen); j++) {
				std::cout << std::hex << (uint64_t) gate->gs.val[j] << std::dec;
			}
			std::cout << std::endl;
#endif
			...
			for(uint32_t n = 0; n < nvals; n++, ctr_idx[outs_id] += choicelen) {
				...
				//std::cout << "Server parent input shares to gate " << m_vTTGates[i][g] << ": " <<  std::hex << parents_value << std::dec << std::endl;
				//std::cout << "Updated choice input: " << std::hex << updated_choice << std::dec << std::endl;

#ifdef DEBUG_SPLUT
				std::cout << "Server parent input shares to gate " << m_vTTGates[i][g] << ": " <<  std::hex << parents_value <<
						", receiver choice = " << updated_choice << std::dec << std::endl;
#endif
				...
				for(uint32_t o = 0; o < out_bits; o++) {
					...
					if(((gate->gs.val[(n+(o*nvals))/typebitlen] >> ((n+(o*nvals))%typebitlen)) & 0x01)) {
						for(uint32_t p = 0; p < iters; p++) {
							//std::cout << std::hex << gateval_buf[p] << " ^ " << gateval_masks[out_bits-1][o][p];;
							gateval_buf[p] ^= gateval_masks[out_bits-1][o][p];
							//std::cout << " = " << gateval_buf[p] << std::dec << std::endl;
						}
					}
				}
//				std::cout << "gateval = " << std::hex << gateval << std::dec << std::endl;
//				std::cout << "gatebuf = ";
//				for(uint32_t p = 0; p < iters; p++) {
//					std::cout << std::hex << gateval_buf[p] << std::dec << ", ";
//				}
//				std::cout << std::endl;

				//std::cout << "Server input gates = " << std::hex << tmp_val << std::dec << std::endl;
...
//					std::cout << std::hex << ((uint64_t*)aes_sbox_multi_seq_perm_out_ttable[p])[2*entry_perm] << ", " <<
//							((uint64_t*)aes_sbox_multi_seq_perm_out_ttable[p])[2*entry_perm+1] << std::dec << std::endl;
//				}
...
//					tableval[t] = tmp_table;
					...
					//std::cout << "table val = " << std::hex << tmp_table  << std::dec << std::endl;
...
#ifdef DEBUGBOOL_NO_MT
						std::cout << "For t = " << t << ", pos " << (tmp_choice+t)%tmp_mod << " send = " << std::hex << tmp_mask << ", table = " << tmp_table << " (tid = " << (tmp_val^t) <<
								"), gval = " << gateval << ", precomp OT = " << tmp_pre_mask << std::dec << std::endl;
#endif
...
//				for(uint32_t p = 0; p < 16; p++) {
//					std::cout << std::hex << ((uint64_t*)tableval)[2*p] << ", " <<
//							((uint64_t*)tableval)[2*p+1] << std::dec << std::endl;
//				}
//				std::cout << "my table:" << std::endl;
//				for(uint32_t p = 0; p < 16; p++) {
//					std::cout << std::hex << ((uint64_t*)mytable)[2*p] << ", " <<
//							((uint64_t*)mytable)[2*p+1] << std::dec << std::endl;
//				}
...
void SetupLUT::ReceiverEvaluateTTGates() {
	...
	for(uint32_t i = 0; i < m_vTTGates.size(); i++) {
		...
		for(uint32_t g = 0; g < m_vTTGates[i].size(); g++) {
...
#ifdef DEBUG_SPLUT
				std::cout << "Value from mask buf = " << std::hex << tmp_rcv << ", precomp OT = " <<
						m_vPreCompOTR[i][outs_id]->Get<uint64_t>((m_vPreCompChoiceIdx[i][outs_id]*out_bits), out_bits) << std::dec << std::endl;
#endif
			...
			for(uint32_t j = 0; j < ceil_divide(gate->nvals, typebitlen); j++) {
				std::cout << (uint64_t) gate->gs.val[j] << std::dec;
			}
			std::cout << std::endl;
#endif
			for(uint32_t j = 0; j < nparents; j++) {
...
#ifdef DEBUG_SPLUT
			std::cout << "Outshare: " << std::hex << gate->gs.val[j] << " = " << m_vGates[parentid].gs.val[j] << " (mine) ^ " <<
					m_vOutputShareRcvBuf.Get<UGATE_T>(rcvshareidx, len) << " (others)" << std::dec << std::endl;
#endif
			rcvshareidx += len;
		}
		UsedGate(parentid);
