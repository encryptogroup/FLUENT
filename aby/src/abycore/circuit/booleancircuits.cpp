...
uint32_t BooleanCircuit::PutY2SCONVGate(uint32_t parentid) {
  std::vector<uint32_t> in(1, parentid);
  uint32_t gateid = m_cCircuit->PutCONVGate(in, 1, S_SPLUT, m_nShareBitLen);
  m_vGates[gateid].depth++;
  UpdateLocalQueue(gateid);
  // a Y input gate cannot be parent to a Y2B gate. Alternatively, put a Boolean
  // input gate
...
uint32_t BooleanCircuit::PutS2YCONVGate(uint32_t parentid) {
  std::vector<uint32_t> in(1, parentid);
  uint32_t gateid = m_cCircuit->PutCONVGate(in, 2, S_YAO, m_nShareBitLen);
  UpdateInteractiveQueue(gateid);
...
std::vector<uint32_t>
BooleanCircuit::PutY2SCONVGate(std::vector<uint32_t> parentid) {
  std::vector<uint32_t> out(parentid.size());
  for (uint32_t i = 0; i < parentid.size(); i++) {
    out[i] = PutY2SCONVGate(parentid[i]);
  }
  return out;
}

std::vector<uint32_t>
BooleanCircuit::PutS2YCONVGate(std::vector<uint32_t> parentid) {
  std::vector<uint32_t> out(parentid.size());
  for (uint32_t i = 0; i < parentid.size(); i++) {
    out[i] = PutS2YCONVGate(parentid[i]);
  }
...
share *BooleanCircuit::PutY2SGate(share *ina) {
  return new boolshare(PutY2SCONVGate(ina->get_wires()), this);
}

share *BooleanCircuit::PutS2YGate(share *ina) {
  return new boolshare(PutS2YCONVGate(ina->get_wires()), this);
}
...
std::vector<uint32_t> BooleanCircuit::PutUniversalCircuitFromFile(
  ...
  BooleanCircuit *splut_circuit = (BooleanCircuit *) sharings[S_SPLUT]->GetCircuitBuildRoutine();
  share *input_share;
  share *output_share;
    ...
    if (line != "") {
      ...
      case 'Y': // MUX Gate
        ...
        if (m_eMyRole == SERVER) {
          wires[tokens[2]] = PutVecANDMUXGate(wires[tokens[0]], wires[tokens[1]],
                                              this->PutSIMDINGate(nvals, p1inputs[p1_counter], SERVER));
        } else {
          wires[tokens[2]] = PutVecANDMUXGate(wires[tokens[0]], wires[tokens[1]],
                                              this->PutSIMDINGate(nvals, 0, SERVER));
        }
      ...
      case 'U': // Universal Gate
        ...
        if (m_eMyRole == SERVER) {
          ...
          wires[tokens[tokens.size() - 1]] =
            PutUniversalGate(input_wires, std::vector<uint32_t>((1 << input_wires.size()), 0));
        }
        p1_counter++;
        break;

      case 'L': // LUT
        pos = line.find_last_of(" \t", std::string::npos);
        tokenize_verilog(line.substr(0, pos), tokens);

#ifdef DEBUG_UC
        std::cout << "Look-up Table " << tokens[tokens.size() - 1] << " = LUT(";
        for (size_t i = 0; i < tokens.size() - 1; i++) {
          std::cout << tokens[i] << ", ";
        }
        std::cout << ") table = " << line.substr(pos + 1, std::string::npos)
                  << std::endl;
#endif
        input_wires.clear();
        for (size_t i = 0; i < tokens.size() - 1; i++) {
        ...
        table = line.substr(pos + 1, std::string::npos);
        table_len = table.size();
        inputs_len = tokens.size() - 1;
        assert(table_len == 1 << inputs_len);

        limbs_len = ceil_divide(table_len, sizeof(uint64_t) * 8);
        ttable = (uint64_t *)calloc(sizeof(uint64_t), limbs_len);

        for (size_t i = 0; i < table_len; i++) {
          const auto limb_i = i / (sizeof(uint64_t) * 8);
          const auto limb_pos = i % (sizeof(uint64_t) * 8);
          if (table[i] == '1')
            ttable[limb_i] |= (1 << limb_pos);
        }

        wires[tokens[tokens.size() - 1]] =
            splut_circuit->PutTruthTableGate(input_wires, 1, ttable);
        break;
      ...
      case 'X': // X Gate
        ...
        if (m_eMyRole == SERVER) {
          tmp = PutCondSwapGate(tmp_wire1, tmp_wire2, this->PutSIMDINGate(nvals, p1inputs[p1_counter], SERVER), true);
        } else {
          tmp = PutCondSwapGate(tmp_wire1, tmp_wire2, this->PutSIMDINGate(nvals, 0, SERVER), true);
        }
      ...
      case 'S': // Yao2SPLUT Gate
#ifdef DEBUG_UC
        std::cout << "Yao2SPLUT Gate " << tokens[1] << " = S(" << tokens[0]
                  << ") " << std::endl;
#endif
        tmp_wire1.clear();
        tmp_wire1.push_back(wires[tokens[0]]);
        input_share = new boolshare(tmp_wire1, this);

        output_share = splut_circuit->PutY2BGate(input_share);

        wires[tokens[1]] = output_share->get_wires()[0];
        break;

      case 'B': // SPLUT2Yao Gate
#ifdef DEBUG_UC
        std::cout << "SPLUT2Yao Gate " << tokens[1] << " = S(" << tokens[0]
                  << ") " << std::endl;
#endif
        tmp_wire1.clear();
        tmp_wire1.push_back(wires[tokens[0]]);
        input_share = new boolshare(tmp_wire1, splut_circuit);

        output_share = PutB2YGate(input_share);
  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      if (line != "") {
        ...
        case 'C': // map the input wires to the gate
        ...
        case 'L': // Parse LUT
          ...
          // next will follow n_inputs input wires.
          ...
          prog_size = 1 << n_inputs;

          ttable = (uint32_t *)calloc(1,ceil_divide((1 << n_inputs) * n_outputs, 8 * 8) * 8);

          {
            ...
            for (int i = 0; i < prog_size; i++) {
              for (int j = 0; j < n_outputs; j++) {
                ...
                ttable[k] =
                  ttable[k] | (tokens.at(ctr + (j * prog_size + i))
                               << (((i)*n_outputs + j) % 64));
          ...
          std::reverse(lut_inputs.begin(), lut_inputs.end());

          // std::cout << "TT: ";
          // for (size_t i = 0; i < ceil_divide((1 << n_inputs) * n_outputs, 8 * 8); i++) {
          //   std::cout << ttable[i] << " ";
          // }
          // std::cout << std::endl;
