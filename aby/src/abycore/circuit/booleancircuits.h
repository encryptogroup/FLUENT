...
public:
  ...
  std::vector<uint32_t> PutANDGate(std::vector<uint32_t> inleft,
  ...
  std::vector<uint32_t> PutXORGate(std::vector<uint32_t> inleft,
                                   std::vector<uint32_t> inright);
  share *PutXORGate(share *ina, share *inb);

  uint32_t PutORGate(uint32_t a, uint32_t b);
  share *PutORGate(share *a, share *b);
  std::vector<uint32_t> PutORGate(std::vector<uint32_t> a,
  ...
  template <class T> uint32_t PutINGate(T val) {
    ...
    gate->gs.ishare.inval =
        (UGATE_T *)calloc(1 * m_nShareBitLen, sizeof(UGATE_T));
  ...
  template <class T> uint32_t PutINGate(T *val, e_role role) {
    ...
    if (role == m_eMyRole) {
      ...
      gate->gs.ishare.inval = (UGATE_T *)calloc(
          ceil_divide(1 * m_nShareBitLen, GATE_T_BITS), sizeof(UGATE_T));
  ...
  template <class T> uint32_t PutSIMDINGate(uint32_t ninvals, T val) {
    ...
    GATE *gate = &(m_vGates[gateid]);
    gate->gs.ishare.inval =
        (UGATE_T *)calloc(ninvals * m_nShareBitLen, sizeof(UGATE_T));
  ...
  template <class T>
  uint32_t PutSIMDINGate(uint32_t ninvals, T *val, e_role role) {
    uint32_t gateid = PutSIMDINGate(ninvals, role);
    if (role == m_eMyRole) {
      // assign value
      GATE *gate = &(m_vGates[gateid]);
      gate->gs.ishare.inval = (UGATE_T *)calloc(
          ceil_divide(ninvals * m_nShareBitLen, GATE_T_BITS), sizeof(UGATE_T));
      memcpy(gate->gs.ishare.inval, val,
             ceil_divide(ninvals * m_nShareBitLen, 8));
      gate->instantiated = true;
    }
    return gateid;
  }
  ...
  /* Unfortunately, a template function cannot be used due to virtual - same for
   * Shared*/
  ...
  /* Unfortunately, a template function cannot be used due to virtual. Call
   * Internal PutINGate*/
  share *PutINGate(uint64_t *val, uint32_t bitlen, e_role role) {
    return InternalPutINGate<uint64_t>(1, val, bitlen, role);
  };
  ...
  /* Unfortunately, a template function cannot be used due to virtual. Call
   * Internal PutSharedINGate -  same for Shared*/
  share *PutSharedINGate(uint64_t *val, uint32_t bitlen) {
    return InternalPutSharedINGate<uint64_t>(1, val, bitlen);
  };
  share *PutSharedINGate(uint32_t *val, uint32_t bitlen) {
  ...
  share *PutYaoSharedSIMDINGate(uint32_t nvals, yao_fields *keys,
                                uint32_t bitlen);
  ...
  uint32_t GetNumB2YGates() { return m_nB2YGates; };
  uint32_t GetNumA2YGates() { return m_nA2YGates; };
  uint32_t GetNumYSwitchGates() { return m_nYSwitchGates; }

  uint32_t GetNumANDGates() { return m_vANDs[0].numgates; };
  ...
  uint32_t GetNumXORVals() { return m_nNumXORVals; };

  uint32_t GetNumXORGates() { return m_nNumXORGates; };

  uint32_t GetNumUNIVGates() { return m_nUNIVGates; }
  ...
  std::vector<uint32_t> PutMulGate(std::vector<uint32_t> a,
                                   std::vector<uint32_t> b, uint32_t resbitlen,
                                   bool depth_optimized = false,
                                   bool vector_ands = false);

  std::vector<uint32_t> PutAddGate(std::vector<uint32_t> left,
                                   std::vector<uint32_t> right,
                                   BOOL bCarry = FALSE);
  share *PutADDGate(share *ina, share *inb);

  std::vector<uint32_t> PutSizeOptimizedAddGate(std::vector<uint32_t> left,
                                                std::vector<uint32_t> right,
                                                BOOL bCarry = FALSE);
  std::vector<uint32_t> PutDepthOptimizedAddGate(std::vector<uint32_t> lefta,
                                                 std::vector<uint32_t> right,
                                                 BOOL bCARRY = FALSE,
                                                 bool vector_ands = false);
  std::vector<uint32_t> PutLUTAddGate(std::vector<uint32_t> lefta,
                                      std::vector<uint32_t> right,
                                      BOOL bCARRY = FALSE);

  std::vector<std::vector<uint32_t>> PutCarrySaveGate(std::vector<uint32_t> a,
                                                      std::vector<uint32_t> b,
                                                      std::vector<uint32_t> c,
                                                      uint32_t inbitlen,
                                                      bool bCarry = FALSE);
  std::vector<std::vector<uint32_t>>
  PutCSNNetwork(std::vector<std::vector<uint32_t>> ins);

  std::vector<uint32_t> PutSUBGate(std::vector<uint32_t> a,
                                   std::vector<uint32_t> b,
                                   uint32_t max_bitlen);
  ...
  uint32_t PutSizeOptimizedGTGate(std::vector<uint32_t> a,
                                  std::vector<uint32_t> b);
  uint32_t PutDepthOptimizedGTGate(std::vector<uint32_t> a,
                                   std::vector<uint32_t> b);
  ...
  std::vector<uint32_t> PutMUXGate(std::vector<uint32_t> a,
                                   std::vector<uint32_t> b, uint32_t s,
                                   BOOL vecand = true);

  share *PutVecANDMUXGate(share *a, share *b, share *s);
  std::vector<uint32_t> PutVecANDMUXGate(std::vector<uint32_t> a,
                                         std::vector<uint32_t> b,
  ...
  std::vector<std::vector<uint32_t>> PutCondSwapGate(std::vector<uint32_t> a,
                                                     std::vector<uint32_t> b,
                                                     uint32_t s,
                                                     BOOL vectorized);
  ...
  std::vector<uint32_t> PutLeftShifterGate(std::vector<uint32_t> val,
                                           uint32_t max_bitlen, uint32_t pos,
                                           uint32_t nvals = 1);

  uint32_t PutIdxGate(uint32_t r, uint32_t maxidx);

  share *PutStructurizedCombinerGate(share *input, uint32_t pos_start,
                                     uint32_t pos_incr, uint32_t nvals);
  uint32_t PutStructurizedCombinerGate(std::vector<uint32_t> input,
                                       uint32_t pos_start, uint32_t pos_incr,
                                       uint32_t nvals);

  uint32_t PutCallbackGate(std::vector<uint32_t> in, uint32_t rounds,
                           void (*callback)(GATE *, void *), void *infos,
                           uint32_t nvals);
  share *PutCallbackGate(share *in, uint32_t rounds,
                         void (*callback)(GATE *, void *), void *infos,
                         uint32_t nvals);

  uint32_t PutUniversalGate(std::vector<uint32_t> input_wires,
                            std::vector<uint32_t> lut_programming);
  std::vector<uint32_t> PutUniversalGate(std::vector<uint32_t> a,
                                         std::vector<uint32_t> b,
                                         std::vector<uint32_t> lut_programming);
  share *PutUniversalGate(share *a, share *b,
                          std::vector<uint32_t> lut_programming);

  uint32_t PutTruthTableGate(std::vector<uint32_t> in, uint32_t out_bits,
                             uint64_t *ttable);
  share *PutTruthTableGate(share *in, uint64_t *ttable);

  std::vector<uint32_t> PutTruthTableMultiOutputGate(std::vector<uint32_t> in,
                                                     uint32_t out_bits,
                                                     uint64_t *ttable);
  share *PutTruthTableMultiOutputGate(share *in, uint32_t out_bits,
                                      uint64_t *ttable);
  std::vector<uint32_t> PutLUTGateFromFile(const std::string filename,
                                           std::vector<uint32_t> inputs);
  share *PutLUTGateFromFile(const std::string filename, share *input);

  share *PutY2BGate(share *ina);
  share *PutB2YGate(share *ina);
  share *PutY2SGate(share *ina);
  share *PutS2YGate(share *ina);
  ...
  uint32_t PutY2SCONVGate(uint32_t parentid);
  uint32_t PutS2YCONVGate(uint32_t parentid);
  ...
  std::vector<uint32_t> PutY2SCONVGate(std::vector<uint32_t> parentid);
  std::vector<uint32_t> PutS2YCONVGate(std::vector<uint32_t> parentid);
  ...
  share *PutB2AGate(share *) {
    std::cerr << "B2A not available for Boolean circuits, please use "
                 "Arithmetic circuits instead"
              << std::endl;
   ...
   */
  std::vector<uint32_t> PutFPGate(const std::string func,
                                  std::vector<uint32_t> inputs, uint8_t bitsize,
                                  uint32_t nvals = 1);
  ...
  std::vector<uint32_t> PutFPGate(const std::string func,
                                  ...
                                  std::vector<uint32_t> inb, uint8_t bitsize,
                                  uint32_t nvals = 1);
   ...
   * \return output wire IDs
   */
  std::vector<uint32_t> PutGateFromFile(const std::string filename,
                                        std::vector<uint32_t> inputs,
                                        uint32_t nvals = 1);
  std::vector<uint32_t>
  PutUniversalCircuitFromFile(const std::string filename,
                              const std::string p1filename,
                              std::vector<uint32_t> p2inputs, uint32_t nvals,
                              std::vector<Sharing *> &sharings);

  /**
   * \brief Get the number of input bits for both parties that a given circuit
   * file expects \param the file name of the circuit \return the number of
   * input bits for both parties
   */
  uint32_t GetInputLengthFromFile(const std::string filename);

  void GetInputLengthFromFile(const std::string filename,
                              uint32_t &client_input, uint32_t &server_input);

  void PutMinIdxGate(share **vals, share **ids, uint32_t nvals,
                     share **minval_shr, share **minid_shr);
  void PutMinIdxGate(std::vector<std::vector<uint32_t>> vals,
                     std::vector<std::vector<uint32_t>> ids,
                     std::vector<uint32_t> &minval,
                     std::vector<uint32_t> &minid);

  void PutMaxIdxGate(share **vals, share **ids, uint32_t nvals,
                     share **maxval_shr, share **maxid_shr);
  void PutMaxIdxGate(std::vector<std::vector<uint32_t>> vals,
                     std::vector<std::vector<uint32_t>> ids,
                     std::vector<uint32_t> &maxval,
                     std::vector<uint32_t> &maxid);

  void PutMultiMUXGate(share **Sa, share **Sb, share *sel, uint32_t nshares,
                       share **Sout);

  uint32_t PutUniversalGateCircuit(std::vector<uint32_t> input_wires,
                                   std::vector<uint32_t> lut_programming);
   ...
   */
  share *PutADDChainGate(std::vector<uint32_t> a, std::vector<uint32_t> b,
                         uint32_t carry_in);

  /**
   * Converts unsigned integer input to floating point number of double
   * precision
   ...
   */
  std::vector<uint32_t> PutConvTypeGate(std::vector<uint32_t> wires,
                                        ConvType *from, ConvType *to,
                                        uint32_t nvals = 1);
   ...
   * @return wires of the converted value
   */
  std::vector<uint32_t> PutUint2FpGate(std::vector<uint32_t> wires,
                                       UINTType *from, FPType *to,
                                       uint32_t nvals = 1);
   ...
   */
  // TODO implement
  // std::vector<uint32_t> PutFp2UintGate(std::vector<uint32_t> wires, FPType*
  // from, UINTType* to);

  /**
   * Computes Prefix Or operation, thus, zeros before first seen 1 and 1s after
   * (e.g. 0010100 => 0011111).
   ...
   * @return shifted value
   */
  std::vector<uint32_t> PutBarrelLeftShifterGate(std::vector<uint32_t> wires,
   ...
   * @return shifted value
   */
  std::vector<uint32_t> PutBarrelRightShifterGate(std::vector<uint32_t> wires,
                                                  std::vector<uint32_t> n);
   ...
   */
  share *PutFPGate(share *in, op_t op, uint8_t bitlen = 0, uint32_t nvals = 0,
                   fp_op_setting s = no_status);
   ...
   * @return result of the operation
   */
  share *PutFPGate(share *in_a, share *in_b, op_t op, uint8_t bitlen = 0,
                   uint32_t nvals = 0, fp_op_setting s = no_status);

private:
  /**
   * When inputting shares with bitlen>1 and nvals convert to regular SIMD in
   * gates
   */
  template <class T>
  share *InternalPutINGate(uint32_t nvals, T val, uint32_t bitlen,
                           e_role role) {
   ...
   */
  template <class T>
  share *InternalPutSharedINGate(uint32_t nvals, T val, uint32_t bitlen) {
    share *shr = new boolshare(bitlen, this);
    assert(nvals <= sizeof(T) * 8);
    T mask = 0;
  ...
  share *InternalPutINGate(uint32_t nvals, T *val, uint32_t bitlen,
    ...
    uint64_t tmpval_bytes =
        std::max(typebyteiters * nvals, (uint32_t)sizeof(T)); // * sizeof(T);
    ...
    for (uint32_t i = 0; i < bitlen; i++) {
      ...
      for (uint32_t j = 0; j < nvals; j++) {
        ...
        tmpval[j / typebitlen] +=
            (((val[j * typebyteiters + i / typebitlen] >> (i % typebitlen)) &
              0x01)
             << (j % typebitlen));
      }
  ...
  share *InternalPutSharedINGate(uint32_t nvals, T *val, uint32_t bitlen) {
    ...
    uint32_t typebyteiters = ceil_divide(bitlen, typebitlen);
    uint64_t tmpval_bytes =
        std::max(typebyteiters * nvals, (uint32_t)sizeof(T));
    ; // * sizeof(T);
    // uint32_t valstartpos = ceil_divide(nvals, typebitlen);
    T *tmpval = (T *)malloc(tmpval_bytes);
  ...
  // first dimension: circuit depth, second dimension: num-inputs, third
  // dimension: out_bitlen
