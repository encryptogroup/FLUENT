...
static const aby_ops_t m_tBenchOps[] = {{OP_XOR, S_BOOL, "xorbool"},
...
int32_t read_test_options(int32_t *argcp, char ***argvp, e_role *role,
                          int32_t *bitlen, uint32_t *secparam,
                          std::string *address, uint16_t *port,
                          int32_t *operation, bool *numbers_only,
                          uint32_t *nops, uint32_t *nruns, uint32_t *threads,
  ...
  parsing_ctx options[] = {
      ...
      {(void *)bitlen, T_NUM, "b",
       "Bit-length of operations, default {8,16,32,64}", false, false},
      ...
      {(void *)operation, T_NUM, "o",
       "Test operation with id (leave out for all operations; for list of IDs "
       "use -l), default: all",
       false, false},
      {(void *)nruns, T_NUM, "i", "Number of iterations of tests, default: 1",
       false, false},
      {(void *)numbers_only, T_FLAG, "v",
       "Omit detailed description, print numbers only (default: false)", false,
       false},
      {(void *)&oplist, T_FLAG, "l", "List the IDs of operations", false,
       false},
      {(void *)no_verify, T_FLAG, "t",
       "No output verification (default: false)", false, false},
      {(void *)detailed, T_FLAG, "d",
       "Give detailed online/setup time and communication (default: false)",
       false, false},
      {(void *)nops, T_NUM, "n", "Number of parallel operations, default: 1",
       false, false},
      {(void *)threads, T_NUM, "h", "Number of threads, default: 1", false,
       false}};

  success = parse_options(argcp, argvp, options,
                          sizeof(options) / sizeof(parsing_ctx));

  if (oplist) {
    std::cout << "Operations with IDs: " << std::endl;
    for (uint32_t i = 0; i < sizeof(m_tBenchOps) / sizeof(aby_ops_t); i++) {
      std::cout << "Operation " << i << ": " << m_tBenchOps[i].opname
                << std::endl;
    }
    exit(0);
  }
...
int32_t bench_operations(aby_ops_t *bench_ops, uint32_t nops, ABYParty *party,
                         uint32_t *bitlens, uint32_t nbitlens, uint32_t nvals,
                         uint32_t nruns, e_role role, uint32_t symsecbits,
                         bool numbers_only, bool no_verify, bool detailed) {
  uint64_t *avec, *bvec, *cvec, *verifyvec, typebitmask = 0;
  uint32_t tmpbitlen, tmpnvals;
  share *shra, *shrb, *shrres, *shrout, *shrsel;
  // Shares for Yao IPP
  share *shray, *shrayr, *shrby, *shrbyr, *shrresy, *shrresyr, *shrouty,
      *shroutyr;
  ...
  if (!numbers_only) {
    ...
    if (!detailed) {
      ...
      std::cout << "Setup Time [ms] / Online Time [ms] / Setup Comm [Byte] / "
                   "Online Comm [Byte] / Non-Linear Ops"
                << std::endl;
    }
    std::cout << "-----------------------------------------------" << std::endl;
  }
  ...
  for (uint32_t i = 0; i < nops; i++) {
    ...
    for (uint32_t b = 0; b < nbitlens; b++) {
      ...
      for (uint32_t r = 0; r < nruns; r++) {
        ...
        Circuit *circ =
            sharings[bench_ops[i].sharing]->GetCircuitBuildRoutine();

        for (uint32_t j = 0; j < nvals; j++) {
          avec[j] = (((uint64_t)rand() << (sizeof(uint32_t) * 8)) + rand()) &
                    typebitmask;
          bvec[j] = (((uint64_t)rand() << (sizeof(uint32_t) * 8)) + rand()) &
                    typebitmask;
        }
        ...
        case OP_ADD:
          if (bench_ops[i].opname.compare("addsobool") == 0) {
            shrres =
                new boolshare(((BooleanCircuit *)circ)
                                  ->PutSizeOptimizedAddGate(shra->get_wires(),
                                                            shrb->get_wires()),
                              circ);
          } else if (bench_ops[i].opname.compare("adddovecbool") == 0) {
            shrres = new boolshare(
                ((BooleanCircuit *)circ)
                    ->PutDepthOptimizedAddGate(shra->get_wires(),
                                               shrb->get_wires(), false, true),
                circ);
        ...
        case OP_MUL:
          if (nvals > 1000 && bench_ops[i].sharing == S_YAO) {
            std::cout << "Yao multiplication ignored due to high memory "
                         "requirement!\t";
            ...
            if (bench_ops[i].opname.compare("muldobool") == 0) {
              shrres = new boolshare(((BooleanCircuit *)circ)
                                         ->PutMulGate(shra->get_wires(),
                                                      shrb->get_wires(), bitlen,
                                                      true),
                                     circ);
        ...
        case OP_CMP:
          if (bench_ops[i].opname.compare("cmpsobool") == 0) {
            ...
            shrres->set_wire_id(
                0, ((BooleanCircuit *)circ)
                       ->PutSizeOptimizedGTGate(shra->get_wires(),
                                                shrb->get_wires()));
          } else {
        ...
        case OP_MUX:
          ...
          shrsel->set_wire_id(0,
                              circ->PutXORGate(shra->get_wire_ids_as_share(0),
                                               shrb->get_wire_ids_as_share(0))
                                  ->get_wire_id(0));

          if (bench_ops[i].opname.compare("muxvecbool") == 0) {
            shrres = new boolshare(bitlen, circ);
            ((BooleanCircuit *)circ)
                ->PutMultiMUXGate(&shra, &shrb, shrsel, 1, &shrres);
          } else if (bench_ops[i].opname.compare("muxbool") == 0) {
            shrres = new boolshare(
                ((BooleanCircuit *)circ)
                    ->PutMUXGate(shra->get_wires(), shrb->get_wires(),
                                 shrsel->get_wire_id(0), false),
                circ);
          ...
          for (uint32_t j = 0; j < nvals; j++)
            verifyvec[j] =
                ((avec[j] & 0x01) ^ (bvec[j] & 0x01)) == 0 ? bvec[j] : avec[j];
          break;
        case OP_Y2B:
          shrres = circ->PutXORGate(shra, shrb);
          circ = bc;
        ...
        case OP_SBOX:
          if (bitlen >= 8) {
            ...
            if (bench_ops[i].opname.compare("sboxsobool") == 0) {
              shrres = new boolshare(
                  AESSBox_Forward_BP_Size_Optimized(shrsel->get_wires(),
                                                    (BooleanCircuit *)circ),
                  circ);
            } else if (bench_ops[i].opname.compare("sboxdovecbool") == 0) {
              shrres =
                  new boolshare(AESSBox_Forward_BP_VecMTs_Optimized(
                                    shrsel->get_wires(), (BooleanCircuit *)circ,
                                    buf_pos_even, buf_pos_odd),
                                circ);
            } else {
              shrres =
                  new boolshare(PutAESSBoxGate(shrsel->get_wires(),
                                               (BooleanCircuit *)circ, false),
                                circ);
            }
            for (uint32_t j = 0; j < nvals; j++)
              verifyvec[j] = (uint64_t)
                  plaintext_aes_sbox[avec[j] &
                                     0xFF]; //(avec[j] + bvec[j]) & typebitmask;
          } else {
            std::cout << "*\t";
            aes_remark = true;
            shrres = shra;
        ...
        if (detailed) {
          std::cout
              << bitlen << "\t" << party->GetTiming(P_SETUP) << "\t"
              ...
              << sharings[bench_ops[i].sharing]->GetNumNonLinearOperations()
              << "\t"
              << sharings[bench_ops[i].sharing]->GetMaxCommunicationRounds()
              << std::endl;
        }
        ...
        if (!no_verify) {
          ...
          for (uint32_t j = 0; j < nvals; j++) {
            if (verifyvec[j] != (cvec[j] & typebitmask)) {
              ...
              std::cout << "\t" << get_role_name(role) << " "
                        << bench_ops[i].opname << ": values[" << j
                        << "]: a = " << avec[j] << ", b = " << bvec[j]
                        << ", c = " << (cvec[j] & typebitmask)
                        << ", verify = " << verifyvec[j] << std::endl;
...
bool run_bench(e_role role, const std::string &address, uint16_t port,
               seclvl seclvl, int32_t operation, int32_t bitlen, uint32_t nvals,
               uint32_t nruns, e_mt_gen_alg mt_alg, uint32_t nthreads,
               bool numbers_only, bool no_verify, bool detailed) {
  ...
  bench_operations(op, nops, party, bitlens, nbitlens, nvals, nruns, role,
                   seclvl.symbits, numbers_only, no_verify, detailed);
...
int main(int argc, char **argv) {
  ...
  read_test_options(&argc, &argv, &role, &bitlen, &secparam, &address, &port,
                    &operation, &numbers_only, &nvals, &nruns, &nthreads,
                    &no_verify, &detailed);

  seclvl seclvl = get_sec_lvl(secparam);

  run_bench(role, address, port, seclvl, operation, bitlen, nvals, nruns,
            mt_alg, nthreads, numbers_only, no_verify, detailed);
