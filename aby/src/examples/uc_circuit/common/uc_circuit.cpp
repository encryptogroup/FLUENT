    ...
    while (getline(file, line)) {
      if (line != "") {
        ...
        if (line.at(0) == 'C') {
          ...
          throw std::runtime_error("Invalid gate type encountered");
        } else {
          throw std::runtime_error("Invalid gate type encountered");
        }
      }
...
int32_t test_universal_circuit(e_role role, char *address, uint16_t port,
                               ...
                               const std::string p2input, uint32_t nruns) {
  ...
  for (uint32_t i = 0; i < nruns; i++) {
    auto intermediate_results = circ->PutUniversalCircuitFromFile(filename, p1filename, p2inputs, nvals, sharings);
    for (auto wire : intermediate_results) {
  ...
  */
  ...
  std::cout << "Setup time / comm: " << party->GetTiming(P_SETUP) << " ms / "
            << party->GetSentData(P_SETUP) + party->GetReceivedData(P_SETUP)
            << " bytes " << std::endl;
  std::cout << "Online time / comm: " << party->GetTiming(P_ONLINE) << " ms / "
