...
int32_t read_test_options(int32_t *argcp, char ***argvp, e_role *role,
                          uint32_t *bitlen, uint32_t *nvals, uint32_t *dim,
                          uint32_t *secparam, std::string *address,
                          uint16_t *port, int32_t *sharing,
                          ePreCompPhase *pre_comp_value) {
  ...
  parsing_ctx options[] = {
      ...
      {(void *)sharing, T_NUM, "g",
       "Minimum sharing (0: Boolean (default); 1: Yao)", false, false},
      {(void *)&int_precomp, T_NUM, "c", "PrecompPhase: 0/1/2/3", false,
       false}};
...
int main(int argc, char **argv) {
  ...
  read_test_options(&argc, &argv, &role, &bitlen, &nvals, &dim, &secparam,
                    &address, &port, &sharing, &precomp_phase_value);
  ...
  test_min_eucliden_dist_circuit(role, address, port, seclvl, nvals, dim,
                                 nthreads, mt_alg, S_ARITH, min_sharing,
                                 precomp_phase_value);
