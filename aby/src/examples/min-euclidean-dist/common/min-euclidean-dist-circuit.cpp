...
int32_t test_min_eucliden_dist_circuit(
    ...
    uint32_t dbsize, uint32_t dim, uint32_t nthreads, e_mt_gen_alg mt_alg,
    e_sharing dstsharing, e_sharing minsharing, ePreCompPhase pre_comp_value) {
  uint32_t bitlen = 8, i, j, temp, tempsum, maxbitlen = 32;
  uint64_t output;
  ABYParty *party =
      new ABYParty(role, address, port, seclvl, maxbitlen, nthreads, mt_alg);
  std::vector<Sharing *> &sharings = party->GetSharings();

  /**
          Setting the precomputation value being passed as the precomputation
     mode of operation. Currently precomputation phases only active for GMW
  ...
  */
  ...
  mindst = build_min_euclidean_dist_circuit(Sshr, Cshr, dbsize, dim, Ssqr, Csqr,
                                            distcirc, (BooleanCircuit *)mincirc,
                                            sharings, minsharing);
  ...
  party->ExecCircuit();
  /**
          Condition check added because in Precomputation store mode. Online
     phase is skipped therefore, potentially calling getclearvalue is not
     possible.
  */
  if (pre_comp_value != ePreCompStore) {
    ...
    std::cout << "Testing min Euclidean distance in "
              << get_sharing_name(dstsharing) << " and "
...
// Build_
share *build_min_euclidean_dist_circuit(share ***S, share **C, uint32_t n,
                                        uint32_t d, share **Ssqr, share *Csqr,
                                        Circuit *distcirc,
  ...
  BooleanCircuit *yaocirc =
      (BooleanCircuit *)sharings[S_YAO]->GetCircuitBuildRoutine();
  ...
  for (i = 0; i < n; i++) {
    ...
    if (minsharing == S_BOOL) {
      distance[i] = yaocirc->PutA2YGate(
          distance[i]); // intermediate A2Y conversion for A2B
...
uint64_t verify_min_euclidean_dist(uint32_t **serverdb, uint32_t *clientquery,
                                   uint32_t dbsize, uint32_t dim) {
  uint32_t i, j;
