...
fn emitCircuit(emit: *EmitVerilog) !void {
    ...
    for (circuit_tags) |tag, index| {
        switch (tag) {
            ...
            .y2s => return error.UnsupportedNodeType,
            .s2y => return error.UnsupportedNodeType,

            .@"and" => try emit.emitTwoInput(index),
            .xor => try emit.emitTwoInput(index),
            .y => return error.UnsupportedNodeType,

            .x => return error.UnsupportedNodeType,

            .lut => return error.UnsupportedNodeType,
            .ulut => return error.UnsupportedNodeType,
        }
    }
