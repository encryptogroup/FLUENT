...
fn parse(
    ...
    // <number of input wires parent a> <number of input wires parent a> <number of output wires>
    {
        const line = (try circuit_reader.readUntilDelimiterOrEof(&buf, '\n')) orelse return error.ExpectedData;
        var iter = std.mem.tokenize(u8, line, " ");
        const num_input_wires_a_raw = iter.next() orelse return error.ExpectedData;
        const num_input_wires_b_raw = iter.next() orelse return error.ExpectedData;
        const num_output_wires_raw = iter.next() orelse return error.ExpectedData;
        ...
        const num_output_wires = try std.fmt.parseInt(Circuit.Node.Index, num_output_wires_raw, 10);

        {
            try parser.circuit_inputs.ensureUnusedCapacity(parser.gpa, num_input_wires_a + num_input_wires_b);
            var i: Circuit.Node.Index = 0;
            while (i < num_input_wires_a + num_input_wires_b) : (i += 1) {
