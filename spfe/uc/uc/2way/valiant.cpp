...
Valiant2Way::Valiant2Way(uint32_t inputs, uint32_t gates, uint32_t outputs, string position, bool outest){
    ...
    if (n <= 4){
        ...
        switch(n){
            ...
            case 2:
                node_number = 0;
                add_edge(poles[0], poles[1], true);
                return;
            case 3:
                node_number = 0;
                add_edge(poles[0], poles[1], true);
                add_edge(poles[1], poles[2], true);
                return;
            case 4:
                ...
                add_edge(nodes[1], poles[2], true);
                add_edge(nodes[1], nodes[2], false);
                add_edge(poles[2], nodes[2], true);
                add_edge(nodes[2], poles[3], true);
                nodes[0]->type = "copy";
                nodes[1]->type = "x";
                nodes[2]->type = "y";
                return;
        ...
        for(int i=0; i < superpole_number-1;i++){
            ...
            add_edge(curr->get_output_node(),sub_left->poles[i], true, false);
            add_edge(curr->get_output_node(),sub_right->poles[i], false, false);
            
            // Connect subgraph poles to nodes
            curr = superpoles[i+1];
            add_edge(sub_left->poles[i], curr->get_input_node(),false, true);
            add_edge(sub_right->poles[i], curr->get_input_node(),false, false);
        }   
    } 
}