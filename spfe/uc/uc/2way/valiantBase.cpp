...
void ValiantBase::print_nodes(ofstream &os){
    ...
    size_t sizes [] = {nodes.size(), poles.size()};
...
void ValiantBase::print_graph(){
    string filename = "graphviz/eug";
    ofstream os;
    os.open(filename);
    os << "digraph{\n";
...
void ValiantBase::create_skeleton(uint32_t inputs, uint32_t gates, uint32_t outputs, string position, bool outest){
    ...
    // First block
    add_edge(poles[0], nodes[0], true);
    add_edge(nodes[0], poles[1], true);
    add_edge(nodes[0], nodes[1], false);
    add_edge(poles[1], nodes[1], true);
    ...
    if(n % 2 == 1){
        ...
        add_edge(nodes[node_number-2], poles[n-2], true);
        add_edge(poles[n-2], nodes[node_number-1], true);
        add_edge(nodes[node_number-2], nodes[node_number-1], false);
        add_edge(nodes[node_number-1], poles[n-1], true);
    }
}
...
void ValiantBase::generic_node_path(uint32_t i, uint32_t j){
    add_edge(nodes[i - 1], poles[j - 1], true);
    add_edge(poles[j - 1], nodes[i], true);
    add_edge(nodes[i - 1], nodes[i], false);
    add_edge(nodes[i], poles[j], true);
    add_edge(nodes[i], nodes[i + 1], false);
    add_edge(poles[j], nodes[i + 1], true);
}


void ValiantBase::add_edge(Node* from, Node* to, bool side){
    add_edge(from,to,side,side);

}

void ValiantBase::add_edge(ValiantBase::Node* from, ValiantBase::Node* to, bool from_side, bool to_side){
    ValiantBase::Edge* e = new ValiantBase::Edge(from, to, from_side, to_side);
    to->in_edges.push_back(e);
    from->out_edges.push_back(e);
}

ValiantBase::Edge* ValiantBase::Node::get_in_edge(bool side){
...
ValiantBase::Edge* ValiantBase::Node::get_out_edge(bool side){
    ValiantBase::Edge* result = 0;
    bool flag = false;
...
ValiantBase::Node* ValiantBase::Node::get_parent(bool side){
    ValiantBase::Edge* e_in = get_in_edge(side);
    if (e_in){
        return e_in->from;
    } else{
        return 0;
...
ValiantBase::Node* ValiantBase::Node::get_child(bool side){
    ValiantBase::Edge* e_out = get_out_edge(side);
    if (e_out){
        return e_out->to;
    } else{
        return 0;
...
void ValiantBase::remove_edge(ValiantBase::Edge* e){
    for(unsigned int i=0; i< e->from->out_edges.size(); i++){
        ValiantBase::Edge* e_out = e->from->out_edges[i];
        if (e_out == e){
            e->from->out_edges.erase(e->from->out_edges.begin()+i);
        }
    }
    for(unsigned int i=0; i< e->to->in_edges.size(); i++){
...
ValiantBase::Edge::Edge(ValiantBase::Node* from, ValiantBase::Node* to, bool from_side, bool to_side){
    this->from = from;
    this->to = to;
...
void ValiantBase::edge_embed_gamma1(DAG* g) {
    ...
    for (unsigned int i = 0;i < g->vertices.size();i++) {
        ...
        if (curr_block_num == child_block_num) {
            ...
            g2->add_edge(g2->vertices[curr_block_num - 1], g2->vertices[child_block_num - 2]);
    ...
    for (DAG* sg : dag1s) {
        for (unsigned int i = 0; i < sg->vertices.size();i++) {
            ...
            if (!curr->out_edges.empty()) {
                ...
                if (side_num == 0) {
                    curr_block_out_rec = curr_block->get_output_node()->get_child(true);
                    child_block_in_rec = child_block->get_input_node()->get_parent(true);
                }
                else {
                    curr_block_out_rec = curr_block->get_output_node()->get_child(false);
                    child_block_in_rec = child_block->get_input_node()->get_parent(false);
                }
                int control_bit;
