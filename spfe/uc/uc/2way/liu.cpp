...
WeakLiu2Way::WeakLiu2Way(uint32_t inputs, uint32_t gates, uint32_t outputs, string position, bool outest) {
    ...
    if (n == 2) {
        ...
        add_edge(poles[0], nodes[0], true, true);
        add_edge(nodes[0], poles[1], true, true);
        return;
    }
    else {
        ...
        for (Superpole* sp : superpoles) {
            ...
            if (i != 0) {
                add_edge(sub_left->poles[sp->number - 1], sp->get_input_node(), false, true);
                add_edge(sub_right->poles[sp->number - 1], sp->get_input_node(), false, false);
            }

            // Connect from current graph to subgraph
            if (i != superpole_number - 1) {
                add_edge(sp->get_output_node(), sub_left->poles[sp->number - 1], true, false);
                add_edge(sp->get_output_node(), sub_right->poles[sp->number - 1], false, false);
            }
            i++;
        }
    }
...
Liu2Way::Liu2Way(uint32_t inputs, uint32_t gates, uint32_t outputs, string position, bool outest) {
    ...
    if (n == 2) {
        ...
        for (Superpole* sp : superpoles) {
            ...
            if (i != 0) {
                Liu2Way::Node* predecessor_left = pole_left->get_parent(true);
                Liu2Way::Node* predecessor_right = pole_right->get_parent(true);
                pole_left->remove_in_edge(true);
                pole_right->remove_in_edge(true);
                add_edge(predecessor_left, sp->get_input_node(), true, true);
                add_edge(predecessor_right, sp->get_input_node(), true, false);
            }

            // Connect from current graph to subgraph
            if (i != superpole_number - 1) {
                Liu2Way::Node* successor_left = pole_left->get_child(true);
                Liu2Way::Node* successor_right = pole_right->get_child(true);
                pole_left->remove_out_edge(true);
                pole_right->remove_out_edge(true);
                add_edge(sp->get_output_node(), successor_left, true, true);
                add_edge(sp->get_output_node(), successor_right, false, true);
            }
            i++;
        }
    }
    ...
    for (DAG::Node* u_g : g->vertices) {
        ...
        if (!u_g->out_edges.empty()) {
            ...
            if (block_u == block_v) {
                ...
                g2->add_edge(g2->vertices[block_u->number - 1], g2->vertices[block_v->number - 1]);
...
void Liu2Way::remove_x_switches() {
    for (Liu2Way::Node* node : nodes) {
        if (node->type == "x" && node->in_edges.size() == 1) {
            ...
            remove_edge(e_in);
            remove_edge(e_out);
            add_edge(e_in->from, e_out->to, e_in->from_side, e_out->to_side);
            ValiantBase::Node* child = e_out->to;
            if (child->type == "y") {
                if ((child->get_parent(true) == child->get_parent(false)) && (child->get_parent(true)->type == "x")) {
                    // Merge them to one node (by removing parent node)
                    ValiantBase::Node* parent = e_in->from;
                    ValiantBase::Edge* parent_left_in_edge = parent->get_in_edge(true);
                    ValiantBase::Edge* parent_right_in_edge = parent->get_in_edge(false);
                    ...
                    add_edge(parent_left_in_edge->from, child, parent_left_in_edge->from_side, true);
                    add_edge(parent_right_in_edge->from, child, parent_right_in_edge->from_side, false);
                    // set control bit correctly (if parents swapped wires, we have to swap too)
                    if (parent->control_bit == 1) {
                        child->control_bit = (child->control_bit + 1) % 2;
                    }
                }
            }
        }
