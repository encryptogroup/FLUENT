...
Graph* benchToGraph(string str_filename) {
    ...
    unordered_map<string, int> circuit_outputs;

    // Use any input wires as an input for gnd LUTs
    string dummy_input;
    ...
    while (getline(is, line)) {
        ...
        if (line.rfind("INPUT", 0) == 0) {
            ...
            if (map.find(name) == map.end()) {
              ...
              throw std::runtime_error("Input already defined");
            }
        }
            ...
            if (circuit_outputs.find(name) == circuit_outputs.end()) {
              circuit_outputs[name] = output_number++;
            } else {
              throw std::runtime_error("Output already defined");
            }
        }
        else {
          // ensure outputs exist
          if (g->output_nodes.size() != output_number) {
            g->output_nodes.resize(output_number, -1);
          }
            ...
            if (map.find(tokens[0]) == map.end()) {
                ...
                if (tokens[2] == "gnd") {
                  string type = "gate";
                  if (circuit_outputs.find(name) != circuit_outputs.end()) {
                    const int output_index = circuit_outputs[name];
                    g->output_nodes[output_index] = number;
                    type = "output";
                  }
                  ...
                  std::reverse(curr_node->inputs.begin(), curr_node->inputs.end());
...
void make_copy_gate_tree(Graph::Node* v) {
    while (v->outputs.size() > 2) {
        v->copy_gates.clear();
        for (unsigned int i = 1; i < v->outputs.size(); i += 2) {
...
Graph* create_random_graph(unsigned int num_inputs, unsigned int num_gates, unsigned int num_outputs, unsigned int max_outdegree, unsigned int max_indegree, string filename) {
    unsigned int num_vertices = num_inputs + num_gates + num_outputs;
    ...
    for (unsigned int i = 0; i < num_vertices - num_outputs;i++) {
        for (unsigned int j = 0; j < max_outdegree; j++) {
            unsigned int random_nr = rand_vertices(gen);
            while (random_nr <= i) {
    ...
    for (unsigned int i = num_inputs; i < num_vertices; i++) {
